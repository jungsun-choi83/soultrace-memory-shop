...
with sync_playwright() as p:
    ...
    expect(page.locator('#shop-dialog')).to_contain_text('예시로 확인하기')
    page.locator('[data-archive="demo-email"][data-email="bori@example.test"]').click()
    page.locator('#archive-email-form button[type="submit"]').click()
    page.locator('#demo-code').wait_for()
    page.locator('[data-archive="fill-code"]').click()
    page.locator('#archive-code-form button[type="submit"]').click()
    page.locator('.archive-account').wait_for()
    page.locator('[data-archive="select"][data-id="demo-bori-birthday"]').click()
    page.locator('[data-archive="apply"]').click()
    page.wait_for_function("!document.querySelector('#shop-dialog').open")
    expect(page.locator('#archive-header-label')).to_have_text('보리의 이야기')
    ok('email demo archive connect')
