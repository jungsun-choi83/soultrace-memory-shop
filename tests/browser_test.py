...
with sync_playwright() as p:
    ...
    expect(page.locator('#shop-dialog')).to_contain_text('같은 이메일')
    expect(page.locator('#shop-dialog')).to_contain_text('이터널빔')
    expect(page.locator('#shop-dialog')).to_contain_text('연동 준비 중')
    page.locator('[data-archive="manual"]').click()
    page.wait_for_function("document.querySelector('#personalize-form')")
    ok('archive screen explains shared email without connecting')
    page.locator('[data-product-card="letter"] [data-action="product"]').first.click()
    expect(page.locator('input[name="petName"]')).to_have_value('')
