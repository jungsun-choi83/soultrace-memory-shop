...
export const FAQS = [
  ['소울트레이스에서 만든 편지를 가져올 수 있나요?', '편지를 받았던 이메일을 입력하고 인증번호를 확인한 뒤, 저장된 이야기를 선택하는 구조입니다. 기본 데모는 예시 이메일과 예시 기록만 사용합니다. 실제 이용에는 SoulTrace 기록·소유권을 확인하는 서버 연결이 필요합니다. 이메일만 입력해서 개인정보가 바로 표시되지는 않습니다.'],
