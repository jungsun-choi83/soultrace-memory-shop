...
export function createArchiveFlow({api,openDialog,closeDialog,dialogHeader,onSelected,onCleared,onCancelled,notify}) {
  ...
  const modeLabel=()=>api.getMode()==='live'?'이메일 확인으로 내 이야기만 안전하게 불러와요.':'로컬 데모 · 실제 메일을 보내거나 고객 기록을 조회하지 않습니다.';
  ...
  function render(){
    ...
    if(step==='email'){
      shell('전에 남긴 이야기가 있나요?',`<p class="archive-intro">소울트레이스에서 편지를 받을 때 사용한 이메일을 입력해주세요.<br>비밀번호나 새 회원가입은 필요하지 않아요.</p><form id="archive-email-form"><label class="field"><span>이야기를 받았던 이메일</span><input name="archiveEmail" type="email" maxlength="254" required autocomplete="email" placeholder="your@email.com" value="${esc(email)}" ${busy?'disabled':''}></label><p class="field-help">이메일 확인 후 접근 가능한 이야기만 보여드려요.<br>이메일 확인은 마케팅 수신 신청이 아닙니다.</p><button class="button button-dark archive-primary" type="submit" ${busy?'disabled':''}>${busy?'확인 메일 요청 중…':'인증번호 받기'} <span>→</span></button></form>${api.getMode()!=='live'?`<div class="demo-entry"><strong>예시로 확인하기</strong><p>실제 이메일 대신 데모 주소를 사용해주세요.</p><button class="demo-chip" type="button" data-archive="demo-email" data-email="${DEMO_EMAIL}">보리 · 이야기 2개</button><button class="demo-chip" type="button" data-archive="demo-email" data-email="nabi@example.test">나비 · 사진 없는 이야기</button><button class="demo-chip" type="button" data-archive="demo-email" data-email="empty@example.test">저장된 이야기 없음</button></div>`:''}`);
    }else if(step==='code'){
